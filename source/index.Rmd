...
- [01. 자료형과 연산자](https://yonseidslab.github.io/pybasic/dtype)
...
- [02. 제어문]()
    ...
    - 반복문: for, list comprehension, while
    - 조건문: if, else, elif 
    - 예외 처리: try, except
...
# 2. [아나콘다 설치](https://www.anaconda.com/)
...
설치가 완료되었다면 점검을 해볼 차례입니다. 윈도우 사용자라면 `win+R` 키를 입력한 후 `cmd`를 입력하면 cmd 창을 엸열 수 있습니다. cmd 혹은 기타 터미널을 열고, `conda --version`을 입력해주세요. 다음과 같이 버전이 출력된다면 무사히 설치가 완료된 것입니다. 버전이 완전히 같지 않아도 괜찮습니다.
...
주피터 노트북을 사용하는 방법과 기본적인 기능들을 알아보겠습니다. 먼저 CMD 창에서 자신이 작업할 디렉토리를 생성하고, 해당 디렉토리로 이동합니다. `mkdir workspace`는 `workspace`라는 디렉토리를 생성하라는 명령이고, `cd workspace`는 `workspace` 디렉토리로 이동하라는 명령어입니다. 그래픽 인터페이스에서 `workspace`라는 이름의 새 폴더를 생성하고, 해당 폴더를 여는 것과 같은 동작입니다.

![workspace](./assets/figs/workspace.png)

![workspaceGUI](./assets/figs/workspaceGUI.png)

이제 이 디렉토리에서 주피터 노트북을 실행해보겠습니다. `jupyter notebook`을 입력하면 주피터 노트북 서버가 실행됩니다. 자동으로 브라우저가 열러서 주피터 노트북이 실행되는 분들도 있을 것이고, 아닌 분들도 있을 것입니다. 자동으로 브라우저가 실행되지 않는다면 아래 표시된 URL 부분을 복사하여 브라우저의 주소창에 입력해주시면 됩니다.
