...
... # gapminder[gapminder.year == 2007] : gapminder 데이터프레임에서 2007년 데이터만 선택
... # .lifeExp : lifeExp 컬럼을 선택
>>> tmp.head()
11    43.828
23    76.423
35    72.301
...
... # gapminder[gapminder.year==2007] : 2007년 데이터만 선택
... # .groupby("continent") : 대륙별로 그루핑
... # .mean() : 그루핑된 데이터의 평균 계산
... # reset_index() : 인덱스를 초기화하고 원래 인덱스를 컬럼으로 내보냄
>>> tmp.head()
...
# gapminder[gapminder.year.isin([1957, 2007])] : 1957년, 2007년의 데이터를 선택
# .groupby(["year","continent"]) : 연도별, 대륙별로 그루핑
# .mean() : 그루핑된 데이터에서 평균 계산
# reset_index() : 인덱스를 초기화하고 원래 인덱스를 컬럼으로 내보냄
>>> tmp.head()
...
```{python eval=F}
>>> tmp = gapminder[gapminder.year==2007].sort_values("gdpPercap", ascending=False).head(10)
... # gapminder[gapminder.year==2007]: year 변수가 2007인 데이터만 선택  
... # .sort_values("gdpPercap", ascending=False): "gdpPercap" 변수의 내림차순으로 데이터프레임을 정렬  
... # .head(10): 가장 위의 10개 행 선택  
>>> tmp
               country continent  year  lifeExp        pop    gdpPercap
...
`sns.boxplot`은 말 그대로 박스플롯을 그리는 함수입니다. barplot과 유사하지만, 분포를 좀 더 명확하게 볼 수 있습니다.
...
`lineplot`은 선 그래프를 그리는 함수입니다. 선 그래프를 그릴 때에도 x와 y가 필요하기 때문에, 바플롯이나 박스플롯을 그릴 때와 같은 코드 패턴을 사용합니다.
...
pointplot은 lineplot과 유사하지만 데이터 포인트까지 플롯에 표현합니다. 라인플롯과 거의 똑같이 활용할 수 있으며, 포인트의 개수가 많지 않다면 라인플롯보다 유용한 경우가 많습니다.
...
... # country가 "Korea, Rep." 또는 "Korea, Dem. Rep." 인 데이터 선택
>>> sns.pointplot(
...     data = tmp,
...     x = 'year',
...     y = 'gdpPercap',
